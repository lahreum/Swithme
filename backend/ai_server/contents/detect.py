...
from contents.yolov3.utils import detect_result, draw_outputs
...
# eye_class_names_path = './contents/data/class_name/eye.names'
...
# eye_class_names = [c.strip() for c in open(eye_class_names_path).readlines()]
...
def detect(image):
    ...
    # eye_boxes, eye_scores, eye_classes, eye_nums = eye_yolo.predict(image_in)
    ...
    # image = draw_outputs(image, (eye_boxes, eye_scores, eye_classes, eye_nums), eye_class_names)
    # result = detect_result((face_boxes, face_scores, face_classes, face_nums), face_class_names)
    # result = detect_result((eye_boxes, eye_scores, eye_classes, eye_nums), eye_class_names)
