<template>
  <div>
    ...
    <div>
      <div class="boardInfo" style="float:left;">
    ...
    <div class="boardContent">
      ...
      <div class="heart">
        좋아요 {{ board.boardLiked }} 개<v-icon @click="addLike" v-if="!isLiked"
          >mdi-heart-outline</v-icon
        ><v-icon @click="deleteLike" v-if="isLiked">mdi-heart</v-icon>
      </div>
    </div>
    <v-divider></v-divider>
    <!-- 댓글 입력창 -->
    ...
    <!-- 댓글 갯수 -->
    <p class="commentInfo">
      <span class="countNumber" style="padding-bottom:20px;">
        {{ replyCnt }}
      </span>
      개의 댓글
    </p>
    ...
    <div style="text-align:center;">
      <div
        v-if="isWriter"
        class="oneBtn"
        @click="$router.push('/community/community-modify')"
      >
        <AppBtnMiddle
          :btnColor="'#673fb4'"
          :btnName="'수정'"
      ...
      <div v-if="isWriter" class="oneBtn" @click="deleteBoard">
        <AppBtnMiddle
          :btnColor="'#424242'"
          :btnName="'삭제'"
          :btnNameColor="'white'"
      ...
      <div class="oneBtn" @click="$router.push('/community')">
        <AppBtnMiddle
          :btnColor="'#616161'"
          :btnName="'목록'"
          :btnNameColor="'white'"
...
export default {
  ...
  computed: {
    ...
    calData() {
      // 보여지고 있는 page의 숫자와 page당 보여질 data의 갯수에 따라서 계산된 startOffset과 endOffset을 이용해 slice 하여 return
  ...
  methods: {
    addLike() {
      ...
      params.append('boardId', this.$store.state.boardDetailId);

      this.$Axios
        .post('community/liked', params, {
          headers: {
    ...
    deleteLike() {
      ...
      this.$Axios
        .delete(
          'community/liked?boardId=' + `${this.$store.state.boardDetailId}`,
          {
            headers: {
              'jwt-auth-token': storage.getItem('jwt-auth-token'),
            },
    ...
    getBoardReplyList() {
      this.$Axios
        .get(
          'community/reply?boardId=' + `${this.$store.state.boardDetailId}`,
          {
            headers: {
              'jwt-auth-token': storage.getItem('jwt-auth-token'),
            },
